class Guest {
	...
	createHTMLToolTip()   {
		...
		const html = `
		  <Tip>
			<div>
				<div class="${this.css_card} card"> </div>
				<h1>${this.type}</h1><hr>
			  	<ul>
	...
	//class="${this.css_card}" style=" float: right;"
	...
	// 2 -creates annex container then puts image container as top_card_guest_id
		...
		this.top_card = this.createChild( "top_card_" + this.id, this.css_card, [ "top_card" ] );
		this.annex.appendChild( this.top_card );
		return {
	...
	createBenefitOption( action ) {
		if (( action == "bribed" && this.type == 'representative' )||( action == "burried" && this.type == 'abbot' ) || ( action == "built" && this.type == 'mechanic' ) )	{
			const no_option = this.createChild( this.no_option_id, "option_box", ["whiteblock"] );

			no_option.innerHTML = _("Do not use discount");
			this.top_card.appendChild(no_option );
			return  this.no_option_id ;
		}

	}
	removeBenefitOption() {
		var element = document.getElementById( this.no_option_id );
